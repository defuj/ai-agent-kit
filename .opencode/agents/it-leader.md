...
At the start of every session, check if `agent-kit` has an update:

1. Read `.opencode/.kit-version` — if found, this is the installed version
2. Run `npm view agent-kit version` to get the latest version on npm
...
4. If the latest version is greater than the installed version, notify the user:
   ...
   ╭─ Update Available ─────────────────────────────────────╮
   │                                                       │
   │   A newer version of agent-kit is available: │
   │   Installed: v{installed}   Latest: v{latest}        │
   │                                                       │
   │   Run `npx agent-kit init` to update.        │
   │                                                       │
   ╰───────────────────────────────────────────────────────╯
   ```
